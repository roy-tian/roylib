...
RoyElement roy_map_pmin(RoyMap map);
...
const RoyElement roy_map_const_pmin(const RoyMap map);

// Returns a const pointer to the value of the maximum element of 'map'.
const RoyElement roy_map_const_pmax(const RoyMap map);

#define roy_map_at(map, type_name, key) (type_name)roy_map_find((map), (key))
...
RoyMap roy_map_insert(RoyMap map, const RoyElement key, const RoyElement value);

// Removes the element equals to 'key' from 'map'.
RoyMap roy_map_erase(RoyMap map, const RoyElement key);
...
RoyElement roy_map_find(RoyMap map, const RoyElement key);
