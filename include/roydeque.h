...
RoyElement roy_deque_pointer(RoyDeque deque, int position);

// Returns an iterator to the element at front end.
RoyElement roy_deque_front(RoyDeque deque);

// Returns an iterator to the element at back end.
RoyElement roy_deque_back(RoyDeque deque);

// Returns an iterator to 'position' in 'deque' where the element takes place.
// (If 'position' is in small half of the deque, iteration will start at head, and vice versa.)
// (Returns NULL if position is out of range.)
const RoyElement roy_deque_const_pointer(const RoyDeque deque, int position);

// Returns an const iterator to the element at front end.
const RoyElement roy_deque_const_front(const RoyDeque deque);

// Returns an const iterator to the element at back end.
const RoyElement roy_deque_const_back(const RoyDeque deque);

// Returns a copy of the element at 'position'. (With boundary check)
// (The behavior is undefined if 'dest' is uninitialized.)
RoyElement roy_deque_element(RoyElement dest, const RoyDeque deque, int position);

// Returns a typed pointer to the element at 'position', NULL if position exceeds.
#define roy_deque_at(deque, type_name, position) (type_name)(roy_deque_pointer((deque), (position)))
...
RoyDeque roy_deque_insert(RoyDeque deque, int position, const RoyElement data);

// Adds an element named 'data' at the beginning of 'deque'.
RoyDeque roy_deque_push_front(RoyDeque deque, const RoyElement data);

// Adds an element named 'data' at the end of 'deque'.
RoyDeque roy_deque_push_back(RoyDeque deque, const RoyElement data);
