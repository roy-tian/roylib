...
char * roy_llong_to_str(char * dest, int64_t number, size_t base, size_t width, bool fill_zero);

// Converts the unsigned integer 'number' into a 'width'-width, 'base' character representation in the string 'dest'.
// (The behavior is undefined if 'dest' or 'width' is insufficient, or 'base' is larger than 36 ([0-9a-z]).)
char * roy_ullong_to_str(char * dest, uint64_t number, size_t base, size_t width, bool fill_zero);
...
uint64_t roy_ullong_ror(uint64_t * number, size_t steps, size_t width);

// Returns the value of the 'width'-width unsigned integer 'number' rotated to the left by 'steps' bit positions.
uint64_t roy_ullong_rol(uint64_t * number, size_t steps, size_t width);
...
bool roy_ullong_prime(uint64_t number);
