...
RoyElement roy_array_pointer(RoyArray array, int position);

// Returns a const void pointer to the element at 'position'. (No boundary check)
const RoyElement roy_array_const_pointer(const RoyArray array, int position);
...
RoyElement roy_array_element(RoyElement dest, const RoyArray array, int position);

// Returns a typed pointer to the element at 'position', or NULL if 'position' is invalid.
#define roy_array_at(array, type_name, position) \
        (type_name)((position) >= 0 && (position) < ((array)->size) ? roy_array_pointer((array), (position)) : NULL)
...
RoyArray roy_array_insert(RoyArray array, int position, const RoyElement data);

// Adds an element named 'data' into 'array' next to the last element, or does nothing if 'array' is full.
// (The behavior is undefined if 'data' is uninitialized.)
RoyArray roy_array_push_back(RoyArray array, const RoyElement data);
