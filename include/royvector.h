...
RoyElement roy_vector_pointer(RoyVector vector, int position);

// Returns a const void pointer to the element at 'position'. (No boundary check)
const RoyElement roy_vector_const_pointer(const RoyVector vector, int position);
...
RoyElement roy_vector_element(RoyElement dest, const RoyVector vector, int position);

// Returns a typed pointer to the element at 'position', or NULL if 'position' is invalid.
#define roy_vector_at(vector, type_name, position) \
        (type_name)((position) >= 0 && (position) < ((vector)->size) ? roy_vector_pointer((vector), (position)) : NULL)
...
RoyVector roy_vector_insert(RoyVector vector, int position, const RoyElement data);

// Adds an element named 'data' into 'vector' next to the last element.
// ('vector' will be extended automatically if it's full.)
// (The behavior is undefined if 'data' is uninitialized.)
RoyVector roy_vector_push_back(RoyVector vector, const RoyElement data);
