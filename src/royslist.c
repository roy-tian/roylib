...
static RoySList node_new(const RoyElement data, size_t element_size);
static void     node_delete(RoySList slist);
static RoySList back(RoySList slist);

RoySList
roy_slist_new(void) {
  RoySList ret = ROY_SLIST(malloc(sizeof(struct _RoySList)));
  ret->data = NULL;
  ret->next = NULL;
  return ret;
}
...
node_new(const RoyElement data,
         size_t       element_size) {
  RoySList ret = ROY_SLIST(malloc(sizeof(RoyElement) + sizeof(struct _RoySList)));
